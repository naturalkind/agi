...
# работа с данными из tg
from parse_tg_11 import ProductCatalogManager
manager = ProductCatalogManager()

from morph_search import AdvancedProductClassifier
...
LLM_SERVER_URL = "https://192.168.1.50:5000/generate"
...
# LLM Server
async def llm_server(messages, bot_type, generation_args={"max_new_tokens":1000, "temperature":0.7}):
    ...
    try:
        ...
        logger.error(f"Failed to query LLM server: {str(e)}")
        return None
...
async def query_ocr_server(user_id, chat_id, message_id, file_id, bot_type):
    """
        ...
        while True:
            try:
                ...
                if message_type == 'text':
                    ...
                    extracted_text = await query_ocr_server(message['user_id'], chat_id, message_id, file_id, bot_type)
                ...
                if text is not None:
                    ...
                    logger.info(f"PIPELINE_WORKER-------------->{message_type} from {bot_type}, type {type(chat_id)}, {chat_id}")
                ...
                if message_type not in ['status_update_video', 'gen_voice', 'gen_image', 'ocr'] and text is not None:
                    ...
                    output = await llm_server(messages, bot_type) 
                    ...
                    # запрос к бд
                    products = manager.db.get_products(category="Бронежилеты", min_confidence=0.7, limit=3)
                    print ("---->", products)
                ...
                if message_type == 'voice':
                    ...
                    if response:
                        ...
                        print ("Z"*100, messages)
                        await sender.send(compress({
                            'bot_type': bot_type,
                            'chat_id': -1002412455053,
                            'text': response,
                            'message_id': message_id,
                            'user_id':message['user_id'],
...
class BaseMessageHandler(tornado.web.RequestHandler):
            ...
            if 'message' in data:
                ...
                if 'forward_from' in message:
                    ...
                    if 'voice' in message:
                        ...
                        #await self.send_about_message(-1002412455053, about_text="Тестовая запись")
    ...
    async def send_about_message(self, chat_id, about_text=""):
        await delete_previous_menu(self.bot_token, chat_id)
        if about_text == "":
            about_text = """
            🤖 *О боте*:
...
async def send_simple_message(chat_id, bot_token, about_text=""):
    if about_text == "":
        about_text = """
        🤖 *О боте*:
        """
...
async def process_responses(receiver, send_message_func):
    while True:
        try:
            ...
            elif response['type'] == 'stat':
                user_id = response['user_id']
                processed_text = response['text']
                format_text = f"<b>вопрос пользователь</b> @{user_id}:\n<code>{response['user_messages']}</code>\n\n<b>ответ бота</b> @{bot_type}:\n<code>{processed_text}</code>"
                
                await send_simple_message(chat_id, bot_token, about_text=format_text)
    ...
    async def _handle_video_callback(self, data: dict):
        ...
        try:
            ...
            if status == 'completed':
                ...
                await self._cleanup_task(task_id, chat_id, message_id, user_id, bot_token)
                update_gen_counts(user_id)
            else:
                await update_status_message(bot_token, chat_id, message_id, "Ошибка генерации видео")
                await self._cleanup_task(task_id, chat_id, message_id, user_id, bot_token)
    ...
    async def _handle_ocr_callback(self, data: dict):
        ...
        try:
            ...
            if status == 'completed':
                ...
                print ("COMPLETED OCR !------------!", task_data, extracted_text) 
                ...
                await self._send_ocr_result(bot_token, chat_id, message_id, extracted_text)
                await self._cleanup_task(task_id, chat_id, message_id, user_id, bot_type, task_type='ocr')
                
            else:
                await update_status_message(bot_token, chat_id, message_id, "Ошибка распознавания текста")
                await self._cleanup_task(task_id, chat_id, message_id, user_id, bot_type, task_type='ocr')
               
        except Exception as e:
            logger.error(f"OCR callback error: {str(e)}")
    
    async def _cleanup_task(self, task_id: str, chat_id: str, message_id: str, user_id: str, bot_type: str, task_type: str = 'video'):
        ...
        await delete_status_message(BOT_TOKENS[bot_type], chat_id, message_id)
...
class UniversalCallbackHandler(tornado.web.RequestHandler):
    ...
    async def post(self):
        try:
            ...
            print ("UNIVERSAL CALLBACK HANDLER ----------->", callback_type, data)
